...
{
    public class HttpSystemProxyValue
    {
    ...
    public class SystemProxyManager
    {
        ...
        protected ProxyInfo originalValues;

        public SystemProxyManager()
        {
            AppDomain.CurrentDomain.ProcessExit += (o, args) => RestoreOriginalSettings();
            if (Environment.UserInteractive && WINAPI.Win32.GetConsoleWindow() != IntPtr.Zero)
            {
                var handler = new WINAPI.Win32.ConsoleEventDelegate(eventType =>
                {
                ...
                WINAPI.Win32.Handler = handler;

                // On Console exit make sure we also exit the proxy
                WINAPI.Win32.SetConsoleCtrlHandler(handler, true);
            }
        }
        ...
        public void RestoreOriginalSettings()
        {
            ...
            using (var reg = Registry.CurrentUser.OpenSubKey(RegKeyInternetSettings, true))
            {
                ...
                if (Environment.OSVersion.Version > windows7Version ||
                    WINAPI.Win32.GetSystemMetrics(smShuttingdown) == 0)
        ...
        protected virtual ProxyInfo GetProxyInfoFromRegistry(RegistryKey reg)
        {
            var pi = new ProxyInfo(false,
        ...
        protected virtual void SaveOriginalProxyConfiguration(RegistryKey reg)
        {
            if (originalValues != null) return;
        ...
        protected static void PrepareRegistry(RegistryKey reg)
        {
        ...
        {
            WINAPI.Win32.InternetSetOption(IntPtr.Zero, InternetOptionSettingsChanged, IntPtr.Zero, 0);
            WINAPI.Win32.InternetSetOption(IntPtr.Zero, InternetOptionRefresh, IntPtr.Zero, 0);
        }

        /// <summary>
        ///     Opens the registry key with the internet settings
        /// </summary>
        protected static RegistryKey OpenInternetSettingsKey()
        {
